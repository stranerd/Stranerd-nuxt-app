...
export class PasswordResetFactory extends BaseFactory<null, { password: string }, Keys> {
	readonly rules = {
		...
		cPassword: {
			required: false,
			rules: [isString, (val: string) => isRequiredIf(val, !!this.password), (val: string) => isShallowEqualTo(val, this.password, 'is not equal to the password'), isLongerThanX(7), isShorterThanX(17)]
		}
	}
